...
for (const name in games) {
    const gbestof = games[name].bestof;    
    
    for (const cat in gbestof) {
        if (gbestof[cat] > bestof[cat][1]) {
            bestof[cat][0] = [name];
            bestof[cat][1] = gbestof[cat];
        } else if (gbestof[cat] == bestof[cat][1]) {
            bestof[cat][0].push(name); // In case of ties
        }
    }
}
